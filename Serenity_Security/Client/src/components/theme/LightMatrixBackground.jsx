...
const LightMatrixBackground = () => {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      context.fillStyle = 'rgba(240, 245, 250, 0.3)';
        ...
        for (let j = 0; j < columns; j++) {
          ...
          if (Math.random() < 0.01) {
            if (cell.opacity === 0 && Math.random() > 0.75) {
              ...
              cell.opacity = Math.random() * 0.15 + 0.05;
            } else if (cell.opacity > 0) {
              cell.opacity = 0;
            }
          }

          context.fillStyle = `rgba(20, 40, 60, ${cell.opacity * 2})`; // Increased opacity for better visibility
...
export default LightMatrixBackground;